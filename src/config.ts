...
type LoadedConfig = Pick<CopseConfig, ConfigKey>;
...
function parseConfigFile(contents: string, filePath: string): LoadedConfig {
  ...
  try {
    ...
    throw new CopseError(`Config file "${filePath}" contains invalid JSON.`, {
      cause: error,
    });
  }

  if (!isRecord(parsed)) {
    throw new CopseError(`Config file "${filePath}" must contain a JSON object.`);
  }
  ...
  for (const key of CONFIG_KEYS) {
    ...
    if (typeof value !== "string") {
      throw new CopseError(`Config value "${key}" in "${filePath}" must be a string.`);
    }
