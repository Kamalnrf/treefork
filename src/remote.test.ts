...
async function createTestRemoteRepo(): Promise<TestRemoteRepo> {
	const originDir = await mkdtemp(join(tmpdir(), "copse-origin-"));
	const storageDir = await mkdtemp(join(tmpdir(), "copse-remote-storage-"));

	await git(originDir, ["init"]);
	await git(originDir, ["config", "user.name", "Copse Tests"]);
	await git(originDir, ["config", "user.email", "copse@example.com"]);
	...
	return { originDir, storageDir, copse };
}
...
describe("remote repo workspace lifecycle", () => {
	...
	test("resolve and remove work for remote workspaces", async () => {
		...
		try {
			...
			await expect(
				repo.copse.workspaces.resolve({ name: "agent" }),
	...
	test("checkpoints work with remote workspaces", async () => {
		...
		try {
			...
			await repo.copse.checkpoints.create({ workspace: "agent", name: "snap" });
	...
	test("config mode is remote", async () => {
		...
		try {
			expect(repo.copse.config.mode).toBe("remote");
		} finally {
			await cleanupTestRemoteRepo(repo);
		}
	});
