...
function isDirtyWorktreeError(error: unknown): error is TreeforkGitError {
  return error instanceof TreeforkGitError && DIRTY_WORKTREE_PATTERN.test(error.stderr);
}

function isMissingBranchError(error: unknown): error is TreeforkGitError {
  return error instanceof TreeforkGitError && MISSING_BRANCH_PATTERN.test(error.stderr);
}
